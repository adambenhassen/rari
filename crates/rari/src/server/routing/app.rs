...
pub async fn handle_app_route(
    ...
    // (cache hit, render, early error) is counted. Not-found matches carry the
    // RAW request path, which crawler garbage would turn into unbounded label
    // cardinality — collapse them to one label.
    let timer_route =
