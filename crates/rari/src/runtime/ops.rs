...
pub async fn op_fetch_with_cache(
    ...
    if let Some(ctx) = request_context {
        ...
        match perform_simple_fetch(&url, &options).await {
            // Any completed HTTP exchange is ok — a 404/500 response is data
            // for the caller, not a fetch failure. Gating ok on 2xx made the
            // JS wrapper throw bare "Fetch failed" for every non-2xx and
            // re-fetch the same URL through the fallback path (double backend
            // hit per miss). Matches the request-context branch above.
